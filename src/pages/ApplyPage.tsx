...
function friendlyApplyError(err: any): string {
  ...
  if (err?.name === 'AuthRetryableFetchError' || lower.includes('failed to fetch')) return 'Network error: please check your internet connection and try again.';
  if (err?.status === 500 || err?.statusCode === 500) return 'Signup reached Supabase, but application processing failed. Please try again in a moment.';
